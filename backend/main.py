...
# Cloud Database mein tables automatically create karne ke liye
models.Base.metadata.create_all(bind=engine)

app = FastAPI(title="Inventory & Order Management System")

# Frontend se communication allow karne ke liye CORS setup
app.add_middleware(
    CORSMiddleware,
    allow_origins=["*"],
...
def create_order(order: schemas.OrderCreate, db: Session = Depends(get_db)):
    ...
    # Rule: Total order amount must be calculated automatically by the backend
    total_amt = product.price * order.quantity
    
    # Rule: Creating an order must automatically reduce available stock
